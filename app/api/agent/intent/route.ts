...
- If the previous assistant gave a command/script/check and the latest user reports "nothing is showing", "blank", "no output", "empty", "did nothing", "why no response", "nothing happened", "see screenshot", "this is what I got", "here", or "same issue", route focused-follow-up. If there is an attached screenshot/image/log, use it only as evidence of the command result. Do not route to standalone screenshot analysis or log comparison.
- If the previous command used shell redirection like "> file.txt 2>&1", the expected answer should explain that output went into the file and tell the user how to read it.
